...
function [x,fval,exitflag,output] = v5_solver(prec,cpt,sep,x_0,timelmt,equal_colloc)
...
%collocation constraints, simply let them all placed on g0
if ~isempty(equal_colloc)
eq_length = length(equal_colloc);

total_var_row8 = zeros(200000,3);

row_neq8 = 1;
temp_row_idx = 1;

for i = 1:eq_length
    tmp_array = equal_colloc{i};
    tmp_length = length(tmp_array);
    if tmp_length == 1
        continue
    else
        for j = 2:tmp_length
            total_var_row8(temp_row_idx,:) = [row_neq8 cpt(tmp_array(1),3) 1];
            total_var_row8(temp_row_idx+1,:) = [row_neq8 cpt(tmp_array(j),3) -1];
            temp_row_idx = temp_row_idx + 2;
            row_neq8 = row_neq8+1;
        end
    end
end
total_var_row8(temp_row_idx:end,:) = []; 
B_neq8 = zeros(row_neq8-1,1);
SP_neq8 = sparse(total_var_row8(:,1),total_var_row8(:,2),total_var_row8(:,3),row_neq8-1,total_var);
end
% extra
...
if ~isempty(equal_colloc)
 [x,fval,exitflag,output] = cplexmilp(f,SP_left_set1,B_right_set1',SP_neq8,B_neq8,[],[],[],lb,ub,ctype,x_0,opt);
else
